...
class User extends Authenticatable
{
    ...
    public function allUsers($data)
    {
        ...
        $query = DB::table($this->getTable())->selectRaw('SQL_CALC_FOUND_ROWS users.*, 
        roles.slug as role_slug, roles.name as role_name'
        )->join(
            'role_users', 'users.id', '=', 'role_users.user_id', 'inner'
            )->join(
                'roles', 'role_users.role_id', '=', 'roles.id', 'inner');
        $number = $data['number'];
        if (!empty($number)) {
            $offset = ($data['page'] - 1) * $number;
            $query->limit($number)->offset($offset);
    ...
    {
        $result = DB::table('usermeta')->select('user_id')->where('meta_key', 'access_token')
        ->where(
        'meta_value', $token)
        ->get()->first();
    ...
    {
        $query = DB::table('roles')->selectRaw('roles.id, roles.slug, roles.name')
    ...
    public function updateUserRole($user_id, $role_id)
    {
        ...
        $role = User::findRoleById($role_id);
        if ($role && $user) {
