...
class CarPrice extends Model
{
    ...
    public function getAllPrices($car_id)
    {
        $sql = DB::table($this->getTable())->where('car_id', $car_id)->orderBy('ID');
        $results = $sql->get();
    ...
    public function getPriceItems($car_id, $start, $end, $status = 'all')
    {
        $start = esc_sql($start);
        $end = esc_sql($end);
        $sql = DB::table($this->getTable())->whereRaw(
        "((car_price.start_date >= {$start} 
        AND car_price.end_date <= {$end}) 
        OR (car_price.start_date <= {$start} 
        AND car_price.end_date >= {$start}) 
        OR (car_price.start_date <= {$end} 
        AND car_price.end_date >= {$end})) 
        AND car_price.car_id = {$car_id}"
    );
        if ($status != 'all') {
